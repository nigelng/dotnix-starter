...
| `pkgsForValidation`              | nixpkgs with google-fonts overlay for app/font validation (`allowUnfreePredicate` for `1password-cli` / `1password`)                                                      |
...
- **Unfree allowlist:** editors/`commonBase` are free. `allowUnfreePredicate` always allows `1password-cli` / `1password`; when a host's Android config has `"enable": true`, all unfree is allowed (Google Android SDK via `androidenv`). Other unfree stays refused on non-Android hosts. Reinstating VS Code HM with `pylanceVscode` would need an allowlist entry (or Android enable).
