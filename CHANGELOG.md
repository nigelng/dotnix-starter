...
- themes: Catppuccin Mocha for p10k, nvim, eza, vivid LS_COLORS, gh-dash, bat, and git delta (mauve accent)

### Changed

- fonts: default `uiFont` to JetBrainsMono Nerd Font Mono; install `jetbrains-mono` instead of `meslo-lg`
...
- firefox: add adguard-browser-assistant to AMO catalog (#36)
...
- Free public Cachix cache `dotnix-starter` in `flake.yml` (SHA-pinned `cachix-action`; `continue-on-error` until cache + `CACHIX_AUTH_TOKEN` exist)
